...
describe('ProjectClarifyPage', () => {
  it('saves nullable unknowns without claiming R0 evidence', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      let body: unknown
      if (url.endsWith('/api/v1/session')) body = { schema_version: '1.15.0', session_token: 'browser-secret' }
      ...
      else body = { schema_version: '1.15.0', project: { schema_version: '1.0.0', project_id: 'project_12345678901234567890123456789012', title: 'Floating airship', prompt: 'A model', prompt_sha256: 'a'.repeat(64), confirmed_at: '2026-07-18T00:00:00Z', confirmed_by: 'user', status: 'intent_confirmed', evidence_mode: 'user_confirmed', brief_evidence_level: null, fabrication_started: false, hardware_actions: false }, draft: null, brief: null, fabrication_started: false, hardware_actions: false }
  ...
  it('requires a separate user approval before creating the R0 Brief', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      else body = { schema_version: '1.15.0', project: { schema_version: '1.0.0', project_id: draft.project_id, title: 'Floating airship', prompt: 'A model', prompt_sha256: 'a'.repeat(64), confirmed_at: '2026-07-18T00:00:00Z', confirmed_by: 'user', status: 'intent_confirmed', evidence_mode: 'user_confirmed', brief_evidence_level: null, fabrication_started: false, hardware_actions: false }, draft, brief: null, fabrication_started: false, hardware_actions: false }
