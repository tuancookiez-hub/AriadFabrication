...
export function RobotCadWorkspace({ projectId }: { projectId: string }) {
  ...
  return (
    <section className="cad-workspace" aria-labelledby="cad-workspace-title">
      ...
      <div className="cad-flow-strip" aria-label="CAD continuation status"><span className="done"><b>3</b><strong>CAD</strong><small>Parts available</small></span><span className="active"><b>4</b><strong>Verify</strong><small>Checking geometry</small></span><span><b>5</b><strong>Slice</strong><small>After verification</small></span><span><b>6</b><strong>Package</strong><small>After slicing</small></span></div>
      ...
      <div className="cad-next-step"><div><strong>Verification has started</strong><p>{validParts}/{manifest.part_count} parts pass the first kernel-validity gate. Interface clearances, support risk, and slicer evidence still have to pass before Package unlocks.</p></div><Link className="secondary-action" to={`/chat?project=${encodeURIComponent(projectId)}`}>Adjust this design with Codex</Link></div>
    </section>
  )
}
