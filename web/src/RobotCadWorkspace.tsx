...
type PrototypePart = { part_id: string; step: string; stl: string; glb: string; glb_sha256: string; kernel_valid: boolean; solid_count: number; volume_mm3: number; manufacturing_orientation?: string; bounds_mm?: { x: number; y: number; z: number } }
  ...
  return <section className="cad-workspace" aria-labelledby="cad-workspace-title">
    ...
    <div className="cad-workspace-layout">
      ...
      <div className="cad-part-preview"><ModelInspector artifact={artifact} />
        {selected ? <div className="cad-part-actions"><div><strong>{label(selected.part_id)}</strong><span>{Math.round(selected.volume_mm3).toLocaleString()} mm³ · {selected.manufacturing_orientation ?? 'orientation pending'}{selected.bounds_mm ? ` · ${selected.bounds_mm.x} × ${selected.bounds_mm.y} × ${selected.bounds_mm.z} mm` : ''}</span></div><a href={`${artifactRoot}/${selected.step}`} download>STEP</a><a href={`${artifactRoot}/${selected.stl}`} download>STL</a></div> : null}
