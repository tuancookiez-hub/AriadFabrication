...
## Report, profile, and content-identity integrity

M4-L local evidence on 2026-07-16: API 1.7.0 adds separate production and interface-fixture schemas for geometry, printability, G-code preflight, printer, material, process, and orientation values. The production printability contract closes its measurements, policy rules, risk policy, tool versions, warning objects, and source/oriented geometry descriptors. Profile JSON rejects duplicate keys, non-finite constants, coercible strings, undeclared fields, and incomplete nested values before typed construction. Geometry, printability, and preflight producers validate complete report values before publication; checksum-verified replay validates the applicable role schema again.

For production package detail, the reader now compares the packaged PartSpec value with the embedded revision spec; printer/material/process/orientation IDs with their profile contents; printability profile IDs with the package; source and oriented geometry filename/size/SHA-256 values with package descriptors; geometry and printability benchmark IDs; and the preflight artifact with package preflight. Applicable direct artifact reads repeat schema and subset identity checks, so requesting a raw report/profile/PartSpec cannot bypass a detail failure. Revision listing intentionally does not parse detail artifacts and therefore remains a root-availability view rather than a sealed detail-integrity index.

All 19 schema assets meta-validate. Six canonical production profiles, 14 complete interface-fixture report/profile files, and 25 ignored production report/profile snapshots passed direct schema replay. Repository replay discovers seven real revisions: six remain available with ten reports, eight profiles, and zero unavailable inspection records; the known stale R2/R4 revision remains correctly invalid. The full pinned backend suite passes 122 tests. Frontend generated-type, test, lint, and production-build gates pass with eight deterministic tests and two environment-gated skips. A clean 168,400-byte wheel resolves all 19 schema assets from its installed data directory.

API 1.7.0 retains five GET paths, 45 schemas, ten date-time occurrences, and a 68,894-byte OpenAPI snapshot with SHA-256 `7de17348d277fd09da0eaa4cf6b3b4962696d9b24749952a173b0d58111f2f6d`. Generated TypeScript remains 36,287 bytes with SHA-256 `7f744f89ce6ba02931a8761a11e52d2b2844ba15f656fc1af9a269789761837b`. Live loopback evidence returned two of six observed fixtures with next offset 2 and `snapshot_consistent: false`, three reports/four profiles/zero unavailable records for the complete fixture, and the exact 178-byte artifact with SHA-256 `7e554f332b4d825518fb978ef721b6f2ed64bd1f0a8143c035c4c957b8ebeab5`, verified-snapshot labelling, and hardware action false.

Research conclusion: a checksum proves byte identity, while a schema proves shape; neither alone proves that multiple files refer to the same design/profile/package. Cross-artifact semantic identity is a separate required gate. The next pre-execution work is visual/accessibility evidence and a frozen no-hardware job-runner contract.
