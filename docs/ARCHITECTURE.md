...
| Browser interface | Vite + React + TypeScript | M4-D implements journey evidence, direct Three.js preview, worker-owned toolpath playback, generated API response types, and bounded persisted-evidence selection |
...
M3 implements one complete printer-independent Golden Part path through R4, and M4-A through M4-D expose its persisted records through a read-only application slice:
...
- `api/repository.py` loads revision-scoped journeys, optional manifests and packages, groups records only through persisted IDs, confines artifact paths, verifies size/checksum before download, and parses only known report/profile roles after a separate bounded checksum gate. Revision listings skip detail-report parsing.
...
- `web/` contains the Vite React/TypeScript client, shipment-style timeline, warning and claim presentation, artifact links, fixture boundary, reduced-motion behavior, bounded Three.js GLB inspection, worker-owned G-code layer playback, a selectable feature/check/finding/profile/artifact evidence explorer, generated OpenAPI response aliases, real-artifact integration tests, lint/type checks, and production build.
