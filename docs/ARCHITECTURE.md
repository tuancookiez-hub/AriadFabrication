...
- `api/models.py` publishes API 1.14.0 with enum-backed evidence records, stateless intake, explicit user-confirmed project intents, nullable clarification drafts, sanitized local-Codex status, browser-session and curated assistant/tool conversation contracts, literal-false fabrication/hardware capabilities, and explicit date-time formats for exposed timestamps. Clarification readiness is derived from a closed requirement set and is not PartSpec confirmation or R0 evidence.
