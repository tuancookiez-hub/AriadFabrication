...
- `api/models.py` publishes API 1.20.0 with enum-backed evidence records, stateless intake, user-confirmed project intents, nullable clarification drafts, explicit PartSpec approval into R0, an R0-bound planning-only Design record, the read-only robot assembly fixture, the closed live L-bracket request/result contract, four non-mutating Codex tools, sanitized local-Codex status, and literal-false hardware capabilities. Clarification readiness and Design-plan readiness are derived; proposal import and plan persistence remain separate explicit user actions. The live L-bracket action creates parameter-bound digital artifacts outside Journey evidence; only the Brief approval action creates Journey evidence.
...
- `api/app.py` exposes loopback-only health, stateless idea intake, bounded revision-list, revision-detail, revision-comparison, and verified-snapshot artifact routes while advertising `hardware_actions: false` in every application contract. Intake performs no persistence, model inference, CAD, process, or hardware action. Revision offset/page-size constraints fail at HTTP validation before discovery; binary, 404, integrity-conflict, and size-limit responses are explicit in OpenAPI.
