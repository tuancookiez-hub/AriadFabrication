...
- `intake.py` captures any bounded non-empty UTF-8 fabrication prompt and applies the current capability policy to an untrusted semantic proposal. Only the exact registered benchmark can expose R2/R4 demo targets; functional CAD, organic mesh, planning, and unsupported routes remain non-executable until matching providers and gates exist.
...
- `api/models.py` publishes API 1.8.0 with enum-backed evidence records, a closed stateless-intake response, literal-false hardware capabilities, and explicit date-time formats for exposed timestamps, so generated browser types cannot widen lifecycle values or enabled hardware state and contract tooling can identify temporal fields.
