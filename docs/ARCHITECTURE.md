...
- `api/models.py` publishes API 1.15.0 with enum-backed evidence records, stateless intake, user-confirmed project intents, nullable clarification drafts, explicit PartSpec approval into R0, sanitized local-Codex status, browser-session and curated assistant/tool conversation contracts, literal-false fabrication/hardware capabilities, and explicit date-time formats for exposed timestamps. Clarification readiness is derived; only the separate approval action invokes the existing Brief gate.
