...
Multi-part products are decomposed before CAD. `AssemblySpec` records separately manufactured parts, purchased-component envelopes, interfaces, rotating axes, clearances, service requirements, unresolved questions, and explicit claim boundaries. Placeholder, manufacturer-sourced, and measured envelopes remain distinct. Part-level evidence and assembly-level evidence advance independently; one valid solid never proves a valid assembly.
...
| API | FastAPI on the existing Python domain | M4-A implements a loopback-only, read-only filesystem boundary; M4-O/M4-P add an internal store, registry, and admission path, but mutation and streaming remain unavailable |
...
| Metadata | SQLite initially | M4-O/M4-P implement the internal transactional execution control store and immutable registered-plan admission; filesystem artifacts remain the evidence source |
...
M4-N defines a control-plane state machine separately from Journey stage state. It admits only the registered Golden Part to R2 or R4, snapshots every approved input identity before acceptance, allows one active and four queued executions, and freezes idempotency, FIFO, cancellation, lease, crash, resource, event, and failure behavior. M4-O implements its internal persistence in a strict SQLite schema with transactional admission/start/cancel/terminal writes, append-only events, fenced leases, stale-run reconciliation, exact schema/integrity checks, and bounded replay. M4-P adds the closed registry and admission coordinator: the caller selects only R2 or R4, while the registry content-identifies the application bundle, fixed assets, locked CAD runtime, host, profiles, PrusaSlicer archive, and complete installation before the store accepts an immutable plan.

No process runner or fabrication mutation surface uses the store or registry yet. Confirmed project-intent persistence is a separate below-R0 record and cannot create Journey evidence. The registry proves the registered content observed by its manifests but is not an OS sandbox. The control plane cannot contact hardware. Therefore the API must not expose a Run endpoint. [The no-hardware execution contract](EXECUTION.md) remains the gate for the future adapter.
...
M3 implements one complete printer-independent Golden Part path through R4, M4-A through M4-M expose its persisted records through a read-only application slice, M4-N freezes the no-hardware execution boundary, M4-O persists that boundary, and M4-P seals target identity and admission without exposing execution through HTTP:

- `domain/spec.py` implements immutable `PartSpec` 1.0.0 and design-readiness rules. `domain/assembly.py` implements `AssemblySpec` 1.0.0 with closed part, component-envelope, interface, reference-integrity, and no-hardware contracts.
...
- `intake.py` captures any bounded non-empty UTF-8 fabrication prompt, publishes the closed strict Structured Output schema for GPT-5.6 classification, decodes bounded UTF-8 JSON with duplicate/non-finite/unknown-field rejection, and applies capability policy to the resulting untrusted semantic proposal. The first provider boundary cannot author a PartSpec. Only the exact registered benchmark can expose R2/R4 demo targets; a general hard-surface prompt may separately attempt non-evidentiary declarative CAD, while organic mesh and downstream fabrication gates remain unavailable.
- `agent_tools.py` publishes the versioned Codex-to-Ariad capability catalog. Five tools are registerable: stateless intake, two bounded evidence reads, an R0-bound Design-plan proposal, and an R0-bound declarative-CAD proposal. Registered execution, cancellation, and arbitrary CAD-source submission remain visible but unavailable with explicit blockers; every descriptor has a closed input schema and literal-false hardware authority.
- `local_codex.py` performs a bounded read-only app-server initialize and `account/read` probe against an explicitly configured native `codex.exe`. It reduces the result to runtime/authentication availability, discards account metadata, registers no tools, starts no thread, mutates no workspace, and exposes no hardware authority.
- `codex_conversation.py` owns one ephemeral local Codex thread in an existing empty workspace with read-only sandbox, no approvals, disabled web search, an empty MCP override, five non-mutating Ariad dynamic tools, one active turn, a 120-second watchdog, and explicit interruption. The fourth tool validates an R0-bound Design-plan proposal; the fifth validates an R0-bound declarative-CAD proposal. Neither tool persists a Design plan, executes geometry, or grants evidence. The adapter emits only bounded assistant-text, tool-name/lifecycle, and generic turn events; tool arguments/results, reasoning, commands, file activity, raw errors, account metadata, and host details are not public contract fields. Authenticated loopback HTTP routes expose this curated stream through an ephemeral Ariad browser-session token; the token is not a Codex credential.
- `schemas/v1/` contains the runtime-allowlisted persisted contracts plus the assembly-planning contract. `assembly-spec.schema.json` is validated by its domain fixture and exposed through a dedicated read-only planning endpoint, but is not yet accepted by the Journey repository. The canonical interface OpenAPI contract is stored beside them but is not a persisted-instance schema.
...
- `cad/declarative.py` defines the versioned closed CSG document and deterministic CadQuery interpreter for general hard-surface attempts. It caps parts, operations, dimensions, positions, text, and primitives; accepts data rather than source code; requires one kernel-valid solid per part; and exports content-addressed assembly/per-part STEP, per-part STL, and GLB preview artifacts. These outputs remain outside Journey R1/R2 until the normal worker, report, lineage, and revision gates adopt them.
...
- `execution/contracts.py` defines immutable request, R2/R4 plan, host/runtime/toolchain snapshots, policy, lease, failure, record, and event values plus pure admission, start, identity binding, cancellation, terminal transition, lease-renewal, and stale-run interruption operations. It launches nothing, persists nothing, streams nothing, and cannot contact hardware.
- `execution/registry.py` resolves only the fixed R2/R4 allowlist, validates bounded manifests, hashes the application/CAD/slicer identities, returns internally selected paths, and re-verifies an accepted plan. `execution/admission.py` preserves idempotent replay/conflict before resolving genuinely new keys. Neither launches a process or exposes HTTP.
...
- `execution/service.py` coordinates one registered queued plan with the sealed CAD/R4 adapters. It owns the shared deadline/cancellation token and lease monitor, binds Journey identity once, and projects a snapshot event only after the persisted Journey/manifest pair is bounded, identity/status checked, schema-checked where available, and hashed. It has no HTTP or hardware surface; filesystem and network isolation remain preconditions for browser mutation.
- `toolchains/v1/` stores generated content manifests for the traced CAD execution runtime and approved PrusaSlicer portable archive/installation. The binaries remain ignored; `scripts/` contains deterministic regeneration/check commands.
...
- `api/models.py` publishes API 1.21.0 with enum-backed evidence records, stateless intake, user-confirmed project intents, nullable clarification drafts, explicit PartSpec approval into R0, an R0-bound planning-only Design record, the read-only robot assembly fixture, the closed live L-bracket and declarative-CAD request/result contracts, five non-mutating Codex tools, sanitized local-Codex status, and literal-false hardware capabilities. Clarification readiness and Design-plan readiness are derived; Design-plan persistence remains a separate explicit user action. Live CAD actions create parameter/document-bound digital artifacts outside Journey evidence; only the Brief approval action creates Journey evidence.
...
- `api/app.py` exposes loopback-only health, authenticated project/Brief/Design actions, local-Codex conversation, R0-bound CAD proposal/generation, stateless idea intake, bounded revision-list/detail/comparison, and verified-snapshot artifact routes while advertising `hardware_actions: false` in every application contract. The general CAD route reads only the latest validated proposal for the current Brief and invokes the closed interpreter; it accepts no command, path, source code, slicer, or hardware selector. Revision offset/page-size constraints fail at HTTP validation before discovery; binary, 404, integrity-conflict, and size-limit responses are explicit in OpenAPI.
