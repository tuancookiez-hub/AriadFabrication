...
- `api/models.py` publishes API 1.10.0 with enum-backed evidence records, a closed stateless-intake response, a sanitized local-Codex status/compatibility response, literal-false hardware capabilities, and explicit date-time formats for exposed timestamps, so generated browser types cannot widen lifecycle values or enabled hardware state and contract tooling can identify temporal fields.
