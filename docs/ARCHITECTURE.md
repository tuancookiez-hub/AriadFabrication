...
- `api/models.py` publishes API 1.13.0 with enum-backed evidence records, stateless intake, explicit user-confirmed project intents, sanitized local-Codex status, browser-session and curated assistant/tool conversation contracts, literal-false fabrication/hardware capabilities, and explicit date-time formats for exposed timestamps.
