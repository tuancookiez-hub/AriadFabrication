...
- `api/models.py` publishes API 1.19.0 with enum-backed evidence records, stateless intake, user-confirmed project intents, nullable clarification drafts, explicit PartSpec approval into R0, an R0-bound planning-only Design record, the read-only robot assembly fixture, four non-mutating Codex tools, sanitized local-Codex status, and literal-false fabrication/hardware capabilities. Clarification readiness and Design-plan readiness are derived; proposal import and plan persistence remain separate explicit user actions, and only the Brief approval action creates evidence.
