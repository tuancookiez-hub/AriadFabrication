...
No process runner or mutation surface uses the store or registry yet. The current CAD and slicer wrappers are synchronous subprocess calls: they have per-command timeouts but not cooperative cancellation, bounded streaming logs, one total wall deadline, enforced import boundaries, or enforceable process-tree memory/network limits. The registry proves the registered content observed by its manifests but is not an OS sandbox. The control plane launches nothing, is not wired into FastAPI, and cannot create Journey evidence or contact hardware. Therefore the GET-only API must not expose a Run endpoint. [The no-hardware execution contract](EXECUTION.md) remains the gate for the future adapter.
...
- `api/models.py` publishes API 1.12.0 with enum-backed evidence records, a closed stateless-intake response, sanitized local-Codex status, browser-session and curated assistant/tool conversation contracts, literal-false hardware capabilities, and explicit date-time formats for exposed timestamps.
