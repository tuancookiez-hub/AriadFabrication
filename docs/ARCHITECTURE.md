...
| Browser interface | Vite + React + TypeScript | M4-J implements journey evidence, bounded revision discovery/pages, bounded persisted-file snapshots, verified artifact delivery, strict persisted lifecycle/scalar/time contracts, direct Three.js preview, worker-owned toolpath playback, generated API response types, bounded persisted-evidence selection, and server-owned revision comparison |
...
M3 implements one complete printer-independent Golden Part path through R4, and M4-A through M4-J expose its persisted records through a read-only application slice:
