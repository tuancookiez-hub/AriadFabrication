...
- `api/models.py` publishes API 1.17.0 with enum-backed evidence records, stateless intake, user-confirmed project intents, nullable clarification drafts, explicit PartSpec approval into R0, an R0-bound planning-only Design record, four non-mutating Codex tools, sanitized local-Codex status, browser-session and curated assistant/tool conversation contracts, literal-false fabrication/hardware capabilities, and explicit date-time formats for exposed timestamps. Clarification readiness and Design-plan readiness are derived; only the separate Brief approval action creates evidence.
