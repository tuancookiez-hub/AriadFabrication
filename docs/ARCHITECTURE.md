...
- `codex_conversation.py` owns one ephemeral local Codex thread in an existing empty workspace with read-only sandbox, no approvals, disabled web search, an empty MCP override, no registered Ariad tools, one active turn, a 120-second watchdog, and explicit interruption. It emits only bounded assistant-text deltas and generic lifecycle events; reasoning, commands, file activity, raw errors, account metadata, and host details are not public contract fields. Authenticated loopback HTTP routes expose this curated stream through an ephemeral Ariad browser-session token; the token is not a Codex credential.
...
- `api/models.py` publishes API 1.11.0 with enum-backed evidence records, a closed stateless-intake response, sanitized local-Codex status, browser-session and curated conversation contracts, literal-false hardware capabilities, and explicit date-time formats for exposed timestamps.
