...
**Progress:** Through M4-O, the project provides a loopback-only read API, six deterministic non-evidentiary journey fixtures, work-bounded revision discovery/pages, bounded persisted-file snapshots, verified artifact delivery, strict persisted lifecycle/scalar/temporal/root/report/profile/schema/lineage/package/content-identity validation, and a React/TypeScript journey shell. The interface renders real GLB previews through a bounded Three.js inspector, parses real G-code in a bounded Web Worker, and lets users select persisted specification features, evidence records, and semantic revision differences. API 1.7.0 examines no more than 5,000 revision-directory entries, retains at most 500 candidates, loads at most a requested 200-summary page, and discloses observed omissions, truncation reasons, fixed ordering, and live-page non-snapshot behavior. It routes root records, inspection JSON, and artifacts through allocation-capped readers; rejects duplicate JSON keys, non-finite constants, excessive node/depth complexity, invalid header metadata, scalar coercion, unsupported lifecycle/evidence values, ownership drift, inconsistent stage/event state, reversed job/revision/stage/event/approval/manifest chronology, fixture/runtime mixing, enabled package hardware state, and manifest decision/approval drift; enforces 19 packaged evidence schemas plus exhaustive stage ownership, acyclic lineage, exact package role/descriptor/input/warning/report/G-code parity; requires production package PartSpec/profile/report/geometry/preflight identities to agree with checksum-verified content; computes bounded server-owned diffs without rerunning stages; and serves no more than 64 MiB from the exact artifact snapshot whose recorded size and SHA-256 passed. Binary/error/header/list-window, closed enum/literal-false, and date-time semantics are generated into the client contract. The committed parent/child pair produces exactly 12 meaningful changes across seven evidence areas while generated IDs and timestamps produce none. M4-M adds skip navigation, route title/focus handling, semantic keyboard tabs, non-actionable unavailable records, non-color check/toolpath cues, dynamic reduced-motion handling, contrast regression tests, and a dedicated Three.js canvas mount that cannot replace React-owned status nodes. M4-N adds three packaged execution-control schemas and pure, immutable semantics for Golden Part R2/R4 admission, canonical idempotency, one-active/four-queued FIFO, cancellation races, fenced leases, stale-run interruption, bounded events/resources, and structured non-manufacturing failures. M4-O adds an internal WAL/FULL SQLite store that transactionally enforces those scheduling, lease, cancellation, interruption, append-only event, integrity, and replay rules while reserving terminal capacity and remaining unreachable from HTTP. It deliberately adds no process runner or endpoint. All 152 backend tests pass in the pinned CAD/slicer environment. Seven ignored real revisions are discovered: six remain available with ten reports, eight profiles, and zero unavailable inspection records, while one stale R2 trace with an orphaned R4 package is correctly invalid. Frontend type-check, lint, twelve deterministic tests, production build, clean-wheel resolution of all 22 schemas plus installed-store create/reopen, and unchanged GET-only HTTP checks pass; two environment-gated real-artifact tests retain separate recorded passes. Automated visual-browser and assistive-technology evidence, policy-enforcing execution, event transport, and pipeline mutation remain incomplete.
...
- Policy-enforcing no-hardware execution adapter and event projection over the internal transactional store.
