...
**Progress:** M4-F provides a loopback-only read API, six deterministic non-evidentiary journey fixtures, verified artifact snapshots, and a React/TypeScript journey shell. The interface renders real GLB previews through a bounded Three.js inspector, parses real G-code in a bounded Web Worker, and lets users select persisted specification features, geometry/printability/preflight checks, findings, profiles, measurements, full artifact checksum records, and semantic revision differences. API 1.2.1 verifies known report/profile checksums before parsing, computes bounded server-owned diffs without rerunning stages, and serves no more than 64 MiB from the exact byte snapshot whose recorded size and SHA-256 passed. Binary/error/header semantics are generated into the client contract; invalid response metadata and oversize artifacts fail closed. The committed parent/child pair produces exactly 12 meaningful changes across seven evidence areas while generated IDs and timestamps produce none. All 84 backend tests pass in the pinned CAD/slicer environment. Frontend type-check, lint, seven deterministic tests, two real-artifact integration tests, production build, and live HTTP checks pass. Automated visual-browser evidence, fully bounded root/inspection JSON reads, event streaming, and pipeline mutation remain incomplete.
