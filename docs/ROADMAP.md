...
**Progress:** M4-E provides a loopback-only read API, six deterministic non-evidentiary journey fixtures, checksum-verified artifact downloads, and a React/TypeScript journey shell. The interface renders real GLB previews through a bounded Three.js inspector, parses real G-code in a bounded Web Worker, and lets users select persisted specification features, geometry/printability/preflight checks, findings, profiles, measurements, and full artifact checksum records. API 1.2 verifies known report/profile checksums before bounded parsing, reports integrity failures without presenting their contents, and computes bounded semantic revision diffs without rerunning evidence-producing stages. The committed parent/child pair produces exactly 12 meaningful changes across seven evidence areas while generated IDs and timestamps produce none. Generated frontend types and CI reject contract drift. All 80 backend tests pass in the pinned CAD/slicer environment. Frontend type-check, lint, seven deterministic tests, two real-artifact integration tests, production build, and live HTTP checks pass. Automated visual-browser evidence, event streaming, artifact-delivery race hardening, and pipeline mutation remain incomplete.
