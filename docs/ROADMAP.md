...
**Progress:** M4-C provides a loopback-only read API, five deterministic non-evidentiary journey fixtures, checksum-verified artifact downloads, and a React/TypeScript journey shell. The interface renders real GLB previews through a bounded Three.js inspector and parses real G-code in a bounded Web Worker for top-down layer playback; both surfaces preserve explicit preview/simulation/physical claim boundaries. A canonical OpenAPI snapshot generates the browser response types, required fields and read-only constants are contract-tested, and backend/frontend CI rejects drift. All 70 backend tests pass in the pinned CAD/slicer environment. Frontend type-check, lint, four deterministic tests, two real-artifact integration tests, production build, and live HTTP checks pass. Automated visual-browser evidence, richer report/feature selection, event streaming, revision comparison, and pipeline mutation remain incomplete.
