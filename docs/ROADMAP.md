...
**Progress:** M4-B provides a loopback-only read API, five deterministic non-evidentiary journey fixtures, checksum-verified artifact downloads, and a React/TypeScript journey shell. The interface now renders real GLB previews through a bounded Three.js inspector and parses real G-code in a bounded Web Worker for top-down layer playback; both surfaces preserve explicit preview/simulation/physical claim boundaries. All 65 backend tests pass in the pinned CAD/slicer environment. Frontend type-check, lint, four deterministic tests, two real-artifact integration tests, production build, and live HTTP checks pass. Automated visual-browser evidence, OpenAPI/TypeScript drift enforcement, richer report/feature selection, event streaming, revision comparison, and pipeline mutation remain incomplete.
