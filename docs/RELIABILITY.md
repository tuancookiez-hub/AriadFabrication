...
The read-only M4-A through M4-M application slices preserve evidence rather than creating it. M4-N adds a tested execution-control contract without making the API writable:
...
- These execution values are reliability requirements, not runtime-enforcement evidence. The current blocking CAD/slicer wrappers do not yet provide cooperative process-tree cancellation, bounded streaming logs, one total wall deadline, enforceable workspace/memory/process/network limits, a transactional store, leases, crash reconciliation, SSE, or a mutation route. Browser execution remains forbidden until the adapter proves those controls.
...
This establishes interface-contract, bounded-discovery/reader/parser, lifecycle/scalar/temporal/root/report/profile/schema/lineage/package/content-identity integrity, renderer-compatibility, semantic-comparison, verified-delivery, accessibility-foundation, presentation-test evidence, and execution-state-machine contract evidence. All 136 backend tests and twelve deterministic frontend tests pass; two environment-gated real-artifact frontend tests have separate recorded passes and are skipped in the ordinary run. A clean wheel installation resolves all 22 packaged schemas: 19 evidence schemas and three execution-control schemas. Seven ignored real revisions are discovered: six are available with ten reports, eight profiles, and no unavailable inspection records; one stale R2 trace with an orphaned R4 package is explicitly invalid. Screenshot-level visual and assistive-technology QA plus policy-enforced end-to-end pipeline execution through the browser remain M4 work; passing these checks still provides no physical fabrication evidence.
