...
The read-only M4-A through M4-J application slices preserve evidence rather than creating it:
...
This establishes interface-contract, bounded-discovery/reader/parser, lifecycle/scalar/temporal integrity, renderer-compatibility, semantic-comparison, verified-delivery, and presentation-test evidence. All 108 backend tests and eight deterministic frontend tests pass; two environment-gated real-artifact frontend tests have separate recorded passes and are skipped in the ordinary run. Complete read-boundary validation of raw PartSpec/package/manifest/report/profile objects, screenshot-level visual/accessibility QA, and real end-to-end pipeline execution through the browser remain M4 work; passing these checks still provides no physical fabrication evidence.
