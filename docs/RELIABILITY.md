...
The read-only M4-A through M4-M application slices preserve evidence rather than creating it. M4-N adds a tested execution-control contract, M4-O adds its internal transactional store, and M4-P adds closed target resolution and admission without making the API writable:
...
- The current blocking CAD/slicer wrappers still do not provide cooperative process-tree cancellation, bounded streaming logs, one total wall deadline, enforceable workspace/memory/process/network limits, SSE, or a mutation route. Browser execution remains forbidden until the adapter proves those controls.
