...
The read-only M4-A through M4-M application slices preserve evidence rather than creating it:
...
This establishes interface-contract, bounded-discovery/reader/parser, lifecycle/scalar/temporal/root/report/profile/schema/lineage/package/content-identity integrity, renderer-compatibility, semantic-comparison, verified-delivery, accessibility-foundation, and presentation-test evidence. All 122 backend tests and twelve deterministic frontend tests pass; two environment-gated real-artifact frontend tests have separate recorded passes and are skipped in the ordinary run. A clean wheel installation resolves all 19 packaged schemas. Seven ignored real revisions are discovered: six are available with ten reports, eight profiles, and no unavailable inspection records; one stale R2 trace with an orphaned R4 package is explicitly invalid. Screenshot-level visual and assistive-technology QA plus real end-to-end pipeline execution through the browser remain M4 work; passing these checks still provides no physical fabrication evidence.
