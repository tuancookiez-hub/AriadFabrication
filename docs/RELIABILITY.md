...
### General declarative-generation note

The authenticated general path asks Codex to propose a closed declarative CSG document bound to the current R0 Brief. The proposal is data, not Python: Ariad rejects unknown fields, excessive parts/operations, out-of-range dimensions or transforms, unsupported primitives, duplicate identifiers, malformed boolean order, and stale R0 bindings before CadQuery runs. The interpreter then requires each exported part to be one kernel-valid solid and records exact proposal/artifact hashes, bounds, solid counts, and preview triangle count.

This is narrower evidence than Journey R1/R2. It proves that Ariad interpreted the recorded bounded document into the recorded digital solids; it does not prove that Codex understood the user's intent, that parts fit or assemble, or that walls, tolerances, orientation, supports, printer profiles, G-code, safety, or physical behavior are acceptable. Its allowed claim is “fresh model-proposed hard-surface digital geometry generated and kernel-checked.” A valid solid must never be labelled printer-ready.
...
M4 preserves persisted evidence through read-only inspection while separately exposing authenticated loopback-only project, conversation, and bounded live-CAD actions. M4-N adds a tested execution-control contract, M4-O adds its internal transactional store, and M4-P adds closed target resolution and admission without exposing the sealed evidence-producing runner or any hardware action:
...
- The canonical OpenAPI snapshot covers read-only evidence plus the authenticated project/conversation/live-CAD actions, marks hardware capability constants and emitted response fields as required, generates the browser response types, and fails backend or frontend checks when either snapshot or generated code drifts.
- API 1.7.0 parses only the recorded geometry, printability, preflight, printer, material, process, and orientation roles after verifying each file against its persisted size and SHA-256, then requires the exact production or interface-fixture role schema. JSON is capped at 2 MiB plus one detection byte, 500 checks, 100 messages/features, 128 top-level inspection fields, depth 16, and 20,000 nodes; missing, oversized, changed, invalid, ambiguous, unsupported, or scalar-coercing sources become explicit unavailable records rather than pass states. Applicable direct artifact reads repeat schema and semantic identity checks before returning the verified snapshot.
...
- Journey, manifest, package, fixture, inspection, and artifact reads use the same bounded-snapshot primitive and never call unbounded `Path.read_text`/`read_bytes`. Root JSON is capped at 32 MiB plus one detection byte, depth 32, and 200,000 nodes. Duplicate object keys and non-finite constants are rejected; finding booleans and package numbers must have their actual JSON types and finite numeric values rather than coercible strings.
- Revision discovery examines at most 5,000 job/revision directory entries and retains at most 500 candidates. HTTP offset is capped at 500, page size defaults to 100 and is capped at 200, and only the selected page loads root JSON. Every response reports observed/returned/omitted counts, next offset, truncation reasons, fixed ordering, and `snapshot_consistent: false`; incomplete discovery is never described as an exact total and live pages may shift if files change between requests.
- Job, stage, event, finding, decision, approval, package, inspection, evidence, and fixture/runtime classifications use closed vocabularies at the repository and generated API boundaries. Stage timestamps/evidence/errors must agree with lifecycle status; every referenced record must retain job/revision/stage ownership; per-stage event sequence numbers are unique; manifest decisions and approvals match the journey; fixture evidence cannot become real; runtime packages require R4; and every package hardware flag is literal `false`. Malformed lifecycle state invalidates detail and artifact reads, while unsupported checksum-verified inspection state becomes unavailable rather than a pass.
- Journey-domain and read-repository text fields require actual JSON/Python strings rather than stringifying numbers, booleans, arrays, or objects. Typed inspection numerics require actual finite JSON numbers. Persisted timestamps require timezone-aware ISO 8601 values; job creation/update, revision materialization, stage start/completion and order, event bounds/sequence order/final state, approval request/decision, record windows, and manifest generation order are checked before detail or artifact delivery. Brief may begin after job creation but before its first revision exists; revision-owned records may not.
- PartSpec, relevant StageEvent, ArtifactManifest, production FabricationPackage, interface-fixture package roots, and all production/interface-fixture report/profile roles must pass one of 19 allowlisted, wheel-packaged Draft 2020-12 schemas. Profile loading rejects duplicate/non-finite JSON and validates before typed construction; geometry, printability, and preflight producers validate before publication. Production detail cross-checks the packaged PartSpec value, four profile identities, printability profile set, exact/oriented geometry descriptors, benchmark identity, and preflight value against checksum-verified artifacts. The read boundary also requires exhaustive stage-owned record references, unique artifact paths, known and acyclic parent lineage that never points to a later stage or time, one successful latest Package attempt, the exact production 19-role subset, exact package/manifest descriptors and package-stage inputs, warning/status parity, a package-report checksum over the exact parsed bytes, and G-code summary checksum/size/filename parity. This validates persisted evidence relationships; it does not rerun any evidence-producing stage.
- Three additional schemas close no-hardware execution requests, records, and events. The Python contract permits only the registered Golden Part R2/R4 targets; snapshots exact plan identities; fixes canonical idempotency, one-active/four-queued FIFO, cancellation-race, lease-expiry, no-auto-resume, event-bound, resource-ceiling, and structured-failure semantics; and makes every hardware and physical-evidence field literal false.
- The internal SQLite store atomically enforces idempotent admission, accepted-sequence FIFO, one active/four queued records, fenced lease ownership, cancellation/terminal contention, stale interruption without resume, immutable terminal state, and append-only event sequencing. Strict schema/application identity, canonical checksums, accepted-plan hashes, DDL fingerprints, integrity/foreign-key checks, database triggers, bounded record/history replay, and reserved terminal capacity fail closed on the tested corruption and race cases.
- The trusted target registry accepts only R2/R4, internally selects every path/provider/profile/tool, hashes the Ariad source/schema bundle, validates fixed benchmark and profile approvals, matches the exact installed distributions to `uv.lock`, verifies curated CPython and traced dependency trees plus `pyvenv.cfg` and startup hooks, captures the 64-bit Windows host identity, and for R4 verifies the pinned PrusaSlicer archive and all 1,119 installation files. Idempotency-aware admission resolves only new keys; replay/conflict can still return the immutable original after local drift. Launch-time re-verification is implemented but cannot be exercised until the adapter exists.
- The CAD runtime manifest is trace evidence, not a sandbox: it proves the exact files observed in the registered lane but does not yet prevent an unmanifested import. Windows release/version/machine are recorded while operating-system DLL bytes remain outside the content manifest. The next adapter must sanitize ambient state and enforce import, process, memory, workspace, deadline, log, and network policies before launch.
- Store checksums establish application-owned corruption and consistency checks, not signatures against a privileged local database editor. Control history has per-execution bounds but no global archive or disk quota, and filesystem Journey publication is not yet atomically coordinated with SQLite events.
- The sealed Golden Part CAD/slicer wrappers still do not provide cooperative process-tree cancellation, bounded streaming logs, one total wall deadline, enforceable workspace/memory/process/network limits, SSE, or a public execution route. Browser-triggered registered R2/R4 execution remains forbidden until that adapter proves those controls. The separate declarative interpreter accepts only validated data and does not authorize executable generated CAD.
...
- The browser shell provides skip navigation, one main landmark, route-specific titles, route-change focus transfer, ordered card headings, polite loading status, and explicit link names. Inspection modes use a keyboard-operable tablist with Arrow/Home/End behavior. An unavailable artifact is rendered as text rather than a link, and persisted report checks include pass/fail text rather than color alone.
- Toolpath travel uses a dashed line and extrusion a solid line as well as distinct colors. The application observes reduced-motion preference changes while open, stops automatic playback when reduction is requested, and keeps manual Previous/Next controls and a visible explanation. Static tests apply the WCAG contrast formula to common normal-text pairs at 4.5:1 and focus indicators at 3:1. This is bounded regression evidence, not full WCAG or assistive-technology conformance.
...
- The Three.js renderer mounts only into a dedicated React-owned-empty host. It no longer replaces the React-managed loading/error overlay, preventing imperative canvas setup from corrupting React's DOM ownership.
...
This establishes interface-contract, bounded-discovery/reader/parser, lifecycle/scalar/temporal/root/report/profile/schema/lineage/package/content-identity integrity, renderer-compatibility, semantic-comparison, verified-delivery, accessibility-foundation, presentation-test evidence, execution-state-machine contract evidence, transactional control-store evidence, and registered-target content-identity evidence. All 163 backend tests and twelve deterministic frontend tests pass; two environment-gated real-artifact frontend tests have separate recorded passes and are skipped in the ordinary run. A clean wheel installation resolves all 22 packaged schemas—19 evidence schemas and three execution-control schemas—and creates/reopens store schema 2 from the installed package on SQLite 3.50.4. Both checkout-local toolchain generators reproduce their committed manifests exactly. Seven ignored real revisions are discovered: six are available with ten reports, eight profiles, and no unavailable inspection records; one stale R2 trace with an orphaned R4 package is explicitly invalid. Screenshot-level visual and assistive-technology QA plus policy-enforced end-to-end pipeline execution through the browser remain M4 work; passing these checks still provides no physical fabrication evidence.
