...
Finish hardening the read-only Fabrication Journey before adding mutation or model-driven generation. Contract drift, evidence replay, revision comparison, artifact delivery, persisted JSON snapshots, and revision discovery/listing now have explicit bounds. The next slice should reject unsupported persisted lifecycle/status/evidence vocabularies instead of rendering arbitrary strings, then close browser-level visual and interaction verification without weakening the M4 evidence contract.

## M4-A through M4-H implemented
...
- Verified all 96 backend tests pass in the pinned CAD/slicer environment. The frontend lockfile passes its release-age and peer policies; OpenAPI-generated-type drift, test and production TypeScript checks, ESLint, eight deterministic tests, and the Vite production build pass. Two environment-gated real-artifact tests retain separate recorded passes and are skipped in the ordinary run. The evidence reader replayed 61 real checks plus four profiles from an existing ignored R4 revision. Live HTTP checks returned a byte-identical artifact snapshot with its integrity headers, a complete 12-change parent-to-child comparison, and a two-record list page that disclosed six observed candidates, next offset 4, `window_limit`, and `hardware_actions: false`.
...
1. Constrain persisted job/stage/finding/package/evidence vocabularies and cross-field state semantics so malformed local records fail closed instead of becoming plausible UI badges.
...
- Several persisted job, event, stage, finding, package, report, and profile status fields are still exposed as non-empty strings. The domain writer constrains many of them, but a malformed or hand-edited local record can reach the UI with an unsupported label.
