...
Finish hardening the read-only Fabrication Journey before adding mutation or model-driven generation. Contract drift, evidence replay, revision comparison, artifact delivery, persisted JSON snapshots, revision discovery/listing, lifecycle/state vocabularies, scalar record types, temporal ordering, committed root schemas, lineage, and fabrication-package parity now fail closed. The next audit slice should define and enforce complete profile/report value schemas and package-to-profile identity checks before closing browser-level visual and interaction verification.

## M4-A through M4-K implemented
...
- Verified all 116 backend tests pass in the pinned CAD/slicer environment. The frontend lockfile passes its release-age and peer policies; OpenAPI-generated-type drift, test and production TypeScript checks, ESLint, eight deterministic tests, and the Vite production build pass. Two environment-gated real-artifact tests retain separate recorded passes and are skipped in the ordinary run. A clean wheel installation resolves and meta-validates every packaged schema outside the checkout. Live HTTP checks returned API 1.6.0, a byte-identical 178-byte artifact snapshot with its integrity headers, and a two-record list page that disclosed six observed candidates, next offset 4, `snapshot_consistent: false`, and `hardware_actions: false`.
...
1. Add complete schemas for geometry/preflight reports and printer/material/process/orientation profiles, enforce the existing printability-report schema during replay, and match package profile IDs plus the packaged PartSpec to checksum-verified artifact contents.
...
- Geometry/preflight report dictionaries, profile values, and some metadata remain generic JSON objects after bounded checksum-verified parsing. Their selected fields are strict, but complete value schemas, package-to-profile identity parity, and packaged-PartSpec content parity are not yet enforced. PartSpec, StageEvent, ArtifactManifest, and production/fixture package roots now use committed schemas, and package-required-artifact/checksum descriptor parity is enforced against the manifest.
