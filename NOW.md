...
Implement the registered target resolver and policy-enforcing no-hardware execution adapter without weakening the read-only evidence boundary. M4-O now supplies the internal transactional control store for the M4-N contract. Browser-triggered execution remains unavailable until admission re-verifies exact identities and the existing blocking CAD/slicer wrappers are made cancellable with bounded logs, process-tree cleanup, one wall deadline, workspace/memory/process limits, and network denial.

## M4-A through M4-O implemented
...
- Added an internal SQLite schema-1 execution control store with WAL/FULL durability, an Ariad application ID, exact metadata/DDL checks, canonical record/event checksums, accepted-plan hashes, strict tables, append-only and immutable-state triggers, one-active/four-queued database guards, monotonic accepted/lease identities, fenced lifecycle writes, atomic cancellation/terminal races, startup stale-lease reconciliation, bounded record/history reads, an 8 MiB event-log ceiling, and reserved terminal capacity. It tolerates concurrent initialization, rejects corruption and coherent semantic drift, and remains unreachable from HTTP. It launches no process, creates no Journey evidence, and cannot contact hardware.
...
- Verified all 152 backend tests pass in the pinned CAD/slicer environment. The frontend lockfile passes its release-age and peer policies; OpenAPI-generated-type drift, test and production TypeScript checks, ESLint, twelve deterministic tests, and the Vite production build pass. Two environment-gated real-artifact tests retain separate recorded passes and are skipped in the ordinary run. The application bundle is 337.16 KiB / 103.68 KiB gzip, CSS is 26.07 KiB / 5.76 KiB gzip, and the lazy Three.js chunk remains 724.46 KiB / 184.72 KiB gzip with the existing size warning. A clean wheel installation resolves and meta-validates all 22 packaged schemas outside the checkout, imports the execution store from `site-packages`, and creates/reopens store schema 1 with SQLite 3.50.4. Live HTTP remains the unchanged GET-only API 1.7.0.
...
1. Implement the trusted R2/R4 target registry and admission resolver that computes and re-verifies every accepted source, lock, runtime, expectation, profile, adapter, and executable identity without accepting caller-selected paths.
2. Replace blocking process capture with a cancellable no-hardware adapter that can run only that registry, bounds logs/workspace/process trees under one deadline, and fails admission when required isolation cannot be enforced.
3. Coordinate immutable Journey snapshot publication with control events, then add adapter-level crash, cancellation, resource-exhaustion, and recovery tests before versioning POST/cancel/SSE contracts or adding a browser Run control.
...
- The persistent control database is internal only. There is no policy-enforcing job runner, event transport, physical calibration data, selected printer, or selected project license; the current API and interface remain read-only M4 foundations.
- Control records are append-only and individually bounded, but global database retention, archival, disk-capacity reservation, and filesystem-Journey/SQLite coordination are not implemented. Checksums and semantic replay detect corruption and inconsistency; they are not signatures against a privileged local attacker.
