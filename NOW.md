...
Finish hardening the read-only Fabrication Journey before adding mutation or model-driven generation. Contract drift, evidence replay, revision comparison, artifact delivery, persisted JSON snapshots, revision discovery/listing, and persisted lifecycle/state vocabularies now fail closed. The next audit slice should stop coercing malformed scalar text and validate timezone-aware temporal records and ordering before closing browser-level visual and interaction verification without weakening the M4 evidence contract.

## M4-A through M4-I implemented
...
1. Validate required persisted strings and timezone-aware timestamps without coercion, including created/updated/start/completion/decision/event chronology and deterministic API date-time contracts.
...
- Persisted identifiers, labels, messages, and timestamps are still parsed by generic string helpers. Some non-string JSON values can be coerced into text, timestamp fields are not yet required to be timezone-aware ISO 8601 values, and cross-record temporal ordering is not yet checked.
