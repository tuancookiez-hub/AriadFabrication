...
Current verification contains 231 backend tests in the pinned CAD/slicer environment, including the three-tool read-only Codex runtime, authenticated conversation routes, curated assistant/tool events, sanitized app-server compatibility probe, immutable agent-tool authority, strict GPT-5.6 output-boundary decoding, universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.12.0 exposes an ephemeral browser-session token and conversation turn/event/cancel routes; these expose no Codex credential and register only stateless intake plus bounded evidence reads. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, fifteen deterministic tests, and production build. A real local Codex turn called `ariad.capture_idea`, returned an honest planning-only answer, and left its workspace empty. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
