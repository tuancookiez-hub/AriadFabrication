...
Open the printed loopback URL, then use **Build session** for the guided prompt → confirmed Brief → component selection → component-aware blueprint → multi-part CAD → compact Verify summary → real per-part layer playback → simplified Package flow. Stage-scoped Codex help stays inside the build. Return to **Technical evidence** for the committed success/failure fixtures and locally available real R4 model/toolpath evidence. The demo remains useful when PrusaSlicer and the optional 1 GiB CAD environment are absent; those are needed only to regenerate real R2/R4 artifacts.
...
Current verification contains 264 passing backend tests after deterministic runtime-manifest regeneration: 262 ordinary/contract tests plus two separately run sealed R4 integrations. Coverage includes confirmed project intent, clarification, explicit R0 Brief persistence, R0-bound Design planning, the four-tool non-mutating Codex runtime, evidence integrity, sealed execution boundaries, assembly contracts, and real sealed R2/R4 integrations. API 1.19.0 exposes the robot assembly-planning fixture: nine separately manufactured parts, two manufacturer-sourced and two placeholder component envelopes, seven interfaces, and explicit evidence boundaries. Frontend verification includes OpenAPI/generated-type drift, TypeScript, lint, 25 deterministic tests with two optional real-artifact skips, and a production build. Live browser QA passes the guided Prompt → R0 → Design-plan flow, assembly screen, Golden R4 Model/Evidence/Toolpath views, and revision comparison without browser warnings or errors. Assistive-technology and physical testing remain outstanding.
