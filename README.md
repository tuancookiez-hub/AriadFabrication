...
| Projects | A user can save an intent, clarify visible unknowns, approve a complete PartSpec into a checksum-bound R0 Journey, and persist an R0-bound Design plan. The plan remains planning-only: no R1 evidence, CAD generation, fabrication, or hardware action exists |
...
Current verification contains 235 passing backend tests in the ordinary environment with 14 optional CAD/slicer skips; 14 focused pinned registry and real sealed R2/R4 tests also pass after deterministic manifest regeneration. Coverage includes confirmed project-intent, clarification-draft, explicit R0 Brief persistence, and R0-bound Design planning, the four-tool non-mutating Codex runtime, authenticated conversation routes, curated assistant/tool events, sanitized app-server compatibility probe, immutable agent-tool authority, strict GPT-5.6 output-boundary decoding, universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.17.0 exposes an ephemeral browser-session token, conversation routes, confirmed intents, nullable clarification drafts, explicit PartSpec confirmation through R0, planning-only Design records, and four non-mutating Codex tools; it registers only stateless intake, bounded evidence reads, and unpersisted R0-bound Design proposals with Codex. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, nineteen deterministic tests, and production build. Real local Codex turns called `ariad.capture_idea` and `ariad.propose_design_plan`; the latter presented every proposal field and left the saved Design plan unchanged. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Live desktop/mobile browser QA now passes the project-to-R0 journey and the real Golden R4 Model, Evidence, and Toolpath views, including layer stepping and keyboard tab navigation; assistive-technology testing remains outstanding.
