...
| Projects | A user can explicitly confirm and durably save a project intent from chat, rediscover it in Projects, and maintain a nullable clarification draft whose unknown requirements remain visible. Intents and drafts are integrity-checked and deliberately carry no R0 Brief evidence or fabrication-started claim |
...
Current verification contains 237 backend tests in the pinned CAD/slicer environment, including confirmed project-intent and clarification-draft persistence, the three-tool read-only Codex runtime, authenticated conversation routes, curated assistant/tool events, sanitized app-server compatibility probe, immutable agent-tool authority, strict GPT-5.6 output-boundary decoding, universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.14.0 exposes an ephemeral browser-session token, conversation routes, confirmed-intent creation/listing, and nullable clarification drafts; it registers only stateless intake plus bounded evidence reads with Codex. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, seventeen deterministic tests, and production build. A real local Codex turn called `ariad.capture_idea`, returned an honest planning-only answer, and left its workspace empty. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
