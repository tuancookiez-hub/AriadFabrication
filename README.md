...
| Execution control | M4-N freezes the no-hardware request/record/event contract; M4-O implements the transactional SQLite store; M4-P adds a closed R2/R4 registry and idempotency-aware admission that content-identify the Ariad application, CAD runtime, host, benchmark assets, profiles, PrusaSlicer archive, and complete slicer installation; no process runner, event transport, mutation endpoint, or browser Run control exists yet |
