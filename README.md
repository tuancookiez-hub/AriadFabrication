...
| Execution control | M4-N freezes the no-hardware request/record/event contract; M4-O implements an internal transactional SQLite store with idempotent FIFO admission, fenced leases, cancellation/crash linearization, append-only bounded events, exact schema/integrity replay, and no HTTP reachability; no process runner, event transport, mutation endpoint, or browser Run control exists yet |
...
|   |-- execution/            No-hardware contracts and internal transactional control store
