...
Open the printed loopback URL, then use **Build session**. The **live L-bracket** path parses dimensions from a judge-entered prompt and generates new parameter-bound STEP, STL, and GLB artifacts with kernel checks and content hashes. The richer robot reference continues through confirmed Brief → component selection → component-aware blueprint → multi-part CAD → compact Verify summary → real per-part layer playback → simplified Package flow. Stage-scoped Codex help stays inside the build. Return to **Technical evidence** for the committed success/failure fixtures and locally available real R4 model/toolpath evidence. PrusaSlicer is only needed to regenerate sliced R4 artifacts; the live bracket requires the pinned CAD extra installed by the quickstart above.

Supported submission platform: Windows 11 x64. Hardware, physical validation, arbitrary generated-code execution, and unsupported prompt families remain deliberately unavailable. Browser-triggered CAD is limited to the closed, validated seven-parameter L-bracket family; unsupported ideas stop at planning. See the [two-minute-fifty-second runbook](docs/DEMO_RUNBOOK.md) and [submission readiness matrix](docs/HACKATHON.md).
...
|---|---|
| Codex agent | The app-server handshake, sanitized status, authenticated loopback conversation routes, bounded text/tool events, cancellation, and React chat are implemented. Four non-mutating tools cover idea capture, evidence reads, and R0-bound Design-plan proposals. A real local Codex turn called the proposal tool without changing the saved plan. The tool cannot persist output; CAD execution and hardware remain unavailable |
| Projects | A user can save an intent, clarify visible unknowns, approve a complete PartSpec into a checksum-bound R0 Journey, and persist an R0-bound Design plan. A confirmed L-bracket Brief may invoke the closed live CAD family; other Design plans remain planning-only |
...
| CAD generation | The registered Golden Part provider creates exact evidence artifacts. Separately, the browser can execute the closed seven-parameter L-bracket family on demand and return fresh STEP, STL, GLB, parameter hashes, and kernel checks without granting Journey R1/R2 evidence |
...
Current automated verification covers confirmed project intent, clarification, explicit R0 Brief persistence, R0-bound Design planning, the four-tool non-mutating Codex runtime, evidence integrity, sealed execution boundaries, assembly contracts, live bounded L-bracket generation, and real sealed R2/R4 integrations. API 1.20.0 exposes the robot assembly-planning fixture and the authenticated live L-bracket generation action. Frontend verification includes OpenAPI/generated-type drift, TypeScript, lint, deterministic tests with optional real-artifact skips, and a production build. Live browser QA covers both the guided robot reference and the new prompt-derived bracket path, including its exact GLB preview, parameter identity, kernel checks, and artifact downloads. Assistive-technology and physical testing remain outstanding.
