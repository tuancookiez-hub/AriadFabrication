...
|---|---|
| Codex agent | The app-server handshake, sanitized status, authenticated loopback conversation routes, bounded text/tool events, cancellation, and React chat are implemented. Codex CLI 0.144.5 completed a real GPT-5.6 Sol turn that called Ariad's bounded idea-capture tool. Exactly three read-only tools are registered; CAD execution and hardware remain unavailable |
...
Current verification contains 234 passing backend tests in the ordinary environment with 14 optional CAD/slicer skips; 14 focused pinned registry and real sealed R2/R4 tests also pass after deterministic manifest regeneration. Coverage includes confirmed project-intent, clarification-draft, explicit R0 Brief persistence, and R0-bound Design planning, the three-tool read-only Codex runtime, authenticated conversation routes, curated assistant/tool events, sanitized app-server compatibility probe, immutable agent-tool authority, strict GPT-5.6 output-boundary decoding, universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.16.0 exposes an ephemeral browser-session token, conversation routes, confirmed intents, nullable clarification drafts, explicit PartSpec confirmation through R0, and planning-only Design records; it registers only stateless intake plus bounded evidence reads with Codex. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, eighteen deterministic tests, and production build. A real local Codex turn called `ariad.capture_idea`, returned an honest planning-only answer, and left its workspace empty. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Live desktop/mobile browser QA now passes the project-to-R0 journey and the real Golden R4 Model, Evidence, and Toolpath views, including layer stepping and keyboard tab navigation; assistive-technology testing remains outstanding.
