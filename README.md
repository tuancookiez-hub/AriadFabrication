...
| Projects | A user can explicitly save an intent, maintain a nullable clarification draft with visible unknowns, and separately approve a complete PartSpec through the existing Brief gate. Successful approval creates a checksum-bound R0 Journey revision; CAD and fabrication remain unstarted |
...
Current verification contains 240 backend tests in the pinned CAD/slicer environment, including confirmed project-intent, clarification-draft, and explicit R0 Brief persistence, the three-tool read-only Codex runtime, authenticated conversation routes, curated assistant/tool events, sanitized app-server compatibility probe, immutable agent-tool authority, strict GPT-5.6 output-boundary decoding, universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.15.0 exposes an ephemeral browser-session token, conversation routes, confirmed intents, nullable clarification drafts, and explicit PartSpec confirmation through R0; it registers only stateless intake plus bounded evidence reads with Codex. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, eighteen deterministic tests, and production build. A real local Codex turn called `ariad.capture_idea`, returned an honest planning-only answer, and left its workspace empty. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Live desktop/mobile browser QA now passes the project-to-R0 journey and the real Golden R4 Model, Evidence, and Toolpath views, including layer stepping and keyboard tab navigation; assistive-technology testing remains outstanding.
