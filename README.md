...
Current verification contains 205 backend tests in the pinned CAD/slicer environment, including universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.8.0 adds one stateless, non-executing intake POST beside the five evidence-replay GET routes. The expanded runtime manifest covers 613 curated CPython files and 2,285 selected-dependency files. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, fourteen deterministic tests, and production build. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
