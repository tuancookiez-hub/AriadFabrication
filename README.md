...
|---|---|
| Codex agent | The app-server handshake, sanitized status, authenticated loopback conversation routes, bounded text events, cancellation, and React chat are implemented. An isolated Codex CLI 0.144.5 completed a real GPT-5.6 Sol turn through local ChatGPT authentication. No fabrication tools are registered |
...
Current verification contains 225 backend tests in the pinned CAD/slicer environment, including authenticated read-only local Codex conversation routes, the internal event boundary, sanitized app-server compatibility probe, immutable agent-tool authority, strict GPT-5.6 output-boundary decoding, universal intake/routing, Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, store/Journey execution coordination, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. API 1.11.0 adds an ephemeral browser-session token and conversation turn/event/cancel routes; these expose no Codex credential and register zero fabrication tools. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, fifteen deterministic tests, and production build. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
