...
| User interface | M4-J provides the read-only journey shell, six deterministic fixtures including a parent/child revision pair, work-bounded revision discovery and pagination, bounded persisted-record and verified-artifact snapshots, strict lifecycle/scalar/timestamp validation, interactive GLB preview, worker-isolated real G-code layer playback, generated API types, a selectable persisted-evidence explorer, and bounded server-owned revision comparison |
