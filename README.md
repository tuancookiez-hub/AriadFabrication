...
| Execution control | M4-N freezes the no-hardware contract; M4-O implements its transactional SQLite store; M4-P seals R2/R4 target identity. The registered R2/R4 lane now launch-reverifies identity, runs Python through an isolated exact-import bootstrap, suppresses command-shell probing, and supervises CAD plus all three PrusaSlicer commands with cancellation, deadlines, bounded logs, memory/process-tree policy, four forced slicer threads, and D-036 workspace controls. The real lane reaches persisted R2 (15 artifacts) and R4 (36 artifacts). Filesystem/network OS isolation, store/Journey event coordination, authenticated mutation/SSE, and browser Run remain unavailable. |
...
Current verification contains 194 backend tests in the pinned CAD/slicer environment, including Windows supervision/capabilities, exact imports, sealed bootstrap/configuration, real sealed R2, and real sealed R4 integrations. The R4 integration proves three successful supervised slicer commands, fixed four-thread arguments, 36 artifacts, and false printer-connected/print-started package fields. The expanded runtime manifest covers 613 curated CPython files and 2,285 selected-dependency files. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, thirteen deterministic tests, and production build. Seven ignored real revisions are discovered: six remain available while one stale R2 directory with an orphaned R4 package is correctly invalid. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
