...
| Execution control | M4-N freezes the no-hardware contract; M4-O implements its transactional SQLite store; M4-P seals R2/R4 target identity. The registered R2 CAD lane now launch-reverifies that identity, starts through an isolated `-I -B -S` exact-path/SHA-256 import bootstrap, suppresses command-shell probing, and runs under Windows Job Object cancellation, deadline, log, memory, process-tree, and D-036 workspace controls. It reaches the existing persisted R2 Journey with 15 artifacts. Filesystem/network OS isolation, sealed R4 slicing, store/event coordination, mutation endpoints, and browser Run remain unavailable. |
...
Current verification contains 192 backend tests in the pinned CAD/slicer environment, including Windows supervisor/capability tests, manifest-import tests, standalone sealed-bootstrap tests, registry-built bootstrap configuration tests, and real sealed/supervised R2 adapter and persisted-pipeline integrations. The expanded runtime manifest covers 613 curated CPython files and 2,285 selected-dependency files. Frontend verification includes OpenAPI/generated-type drift, dependency policy, TypeScript, lint, thirteen deterministic tests, and production build. Seven ignored real revisions are discovered: six remain available with ten reports, eight profiles, and zero unavailable inspection records, while one stale R2 directory with an orphaned R4 package is correctly invalid. Two environment-gated real-artifact frontend tests retain separate recorded passes. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
