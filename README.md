...
| User interface | M4-G provides the read-only journey shell, six deterministic fixtures including a parent/child revision pair, bounded persisted-record and verified-artifact snapshots, interactive GLB preview, worker-isolated real G-code layer playback, generated API types, a selectable persisted-evidence explorer, and bounded server-owned revision comparison |
...
The active milestone is **M4 - Fabrication Journey interface**. M4-G can list and inspect real persisted R2/R4 revisions, orbit the actual GLB preview, replay the actual linear G-code moves by layer off the UI thread, and demonstrate complete, `needs_input`, failed Geometry, failed Printability, incomplete Package, and parent-to-child revision states with explicitly non-evidentiary fixtures. The generated API 1.2.2 contract carries bounded, checksum-verified evidence into a selectable explorer, computes bounded semantic diffs, and serves artifacts from the exact verified byte snapshot rather than reopening a path after verification. Root records, inspection sources, and artifacts now share allocation-capped reads; duplicate JSON keys, non-finite constants, excessive depth/nodes, and string coercion into boolean/numeric evidence fail closed. Comparison omits volatile IDs and timestamps, runs on the server, and neither reruns stages nor proves equivalence. Event streaming, browser-triggered execution, and screenshot-level browser QA remain incomplete. R4 is still a digital, profile-specific result: physical adhesion, accuracy, stake fit, strength, weathering, food safety, and print success remain unproven.

Current verification passes 91 backend tests in the pinned CAD/slicer environment plus OpenAPI snapshot drift, generated-type drift, frontend dependency-policy, type-check, lint, seven deterministic tests, two real-artifact integration tests, production-build, and live HTTP artifact/comparison checks. Automated visual browser QA remains outstanding for the reason recorded in `NOW.md`.
